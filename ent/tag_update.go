...
func (_u *TagUpdate) AddMonitors(v ...*Monitor) *TagUpdate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
func (_u *TagUpdate) sqlSave(ctx context.Context) (_node int, err error) {
	...
	if _u.mutation.MonitorsCleared() {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2M,
			Inverse: true,
			Table:   tag.MonitorsTable,
			Columns: tag.MonitorsPrimaryKey,
			Bidi:    false,
			Target: &sqlgraph.EdgeTarget{
...
// AddMonitors adds the "monitors" edges to the Monitor entity.
func (_u *TagUpdateOne) AddMonitors(v ...*Monitor) *TagUpdateOne {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
// ClearMonitors clears all "monitors" edges to the Monitor entity.
func (_u *TagUpdateOne) ClearMonitors() *TagUpdateOne {
	_u.mutation.ClearMonitors()
	return _u
}

// RemoveMonitorIDs removes the "monitors" edge to Monitor entities by IDs.
func (_u *TagUpdateOne) RemoveMonitorIDs(ids ...int) *TagUpdateOne {
	_u.mutation.RemoveMonitorIDs(ids...)
	return _u
}

// RemoveMonitors removes "monitors" edges to Monitor entities.
func (_u *TagUpdateOne) RemoveMonitors(v ...*Monitor) *TagUpdateOne {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
