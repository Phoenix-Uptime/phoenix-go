...
func (_q *MonitorQuery) QueryUser() *UserQuery {
...
func (_q *MonitorQuery) QueryStats() *MonitorStatQuery {
	query := (&MonitorStatClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *MonitorQuery) QueryTags() *TagQuery {
...
func (_q *MonitorQuery) WithUser(opts ...func(*UserQuery)) *MonitorQuery {
	query := (&UserClient{config: _q.config}).Query()
...
func (_q *MonitorQuery) WithStats(opts ...func(*MonitorStatQuery)) *MonitorQuery {
	query := (&MonitorStatClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MonitorQuery) WithTags(opts ...func(*TagQuery)) *MonitorQuery {
	query := (&TagClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MonitorQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Monitor, error) {
	...
	if query := _q.withUser; query != nil {
		if err := _q.loadUser(ctx, query, nodes, nil,
			func(n *Monitor, e *User) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withStats; query != nil {
		if err := _q.loadStats(ctx, query, nodes,
			func(n *Monitor) { n.Edges.Stats = []*MonitorStat{} },
			func(n *Monitor, e *MonitorStat) { n.Edges.Stats = append(n.Edges.Stats, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *MonitorQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Monitor, init func(*Monitor), assign func(*Monitor, *User)) error {
...
func (_q *MonitorQuery) loadStats(ctx context.Context, query *MonitorStatQuery, nodes []*Monitor, init func(*Monitor), assign func(*Monitor, *MonitorStat)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Monitor)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(monitorstat.FieldMonitorID)
	}
	query.Where(predicate.MonitorStat(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(monitor.StatsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *MonitorQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Monitor, init func(*Monitor), assign func(*Monitor, *Tag)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(monitor.TagsTable)
		s.Join(joinT).On(s.C(tag.FieldID), joinT.C(monitor.TagsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(monitor.TagsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(monitor.TagsPrimaryKey[0]))
...
func (_q *MonitorQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withUser != nil {
			_spec.Node.AddColumnOnce(monitor.FieldUserID)
		}
	}
